...
class AutoWriter {
    ...
    createAssociations() {
        let str = "";
        const fkTables = lodash_1.default.keys(this.foreignKeys).sort();
        fkTables.forEach(t => {
            const [schemaName, tableName] = types_1.qNameSplit(t);
            const modelName = types_1.recase(this.options.caseModel, tableName);
            ...
            fkFieldNames.forEach(fkFieldName => {
                ...
                if (spec.isForeignKey) {
                    const targetModel = types_1.recase(this.options.caseModel, spec.foreignSources.target_table);
                    const sourceProp = types_1.recase(this.options.caseProp, fkFieldName);
                    ...
                    if (spec.isPrimaryKey) {
                        ...
                        if (otherKey) {
                            const otherModel = types_1.recase(this.options.caseModel, otherKey.foreignSources.target_table);
                            const otherProp = types_1.recase(this.options.caseProp, otherKey.source_column);
